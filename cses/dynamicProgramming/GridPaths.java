...
public class GridPaths{
	...
	private static int countPaths(char grid[][]){
		...
		if(grid[grid.length-1][grid[0].length-2] == '*' && grid[grid.length-2][grid[0].length-1] == '*') return 0;
		...
		//Preprocessing
		for(int i = dp.length-2; i > -1 && grid[i][grid[0].length-1] != '*'; i--)dp[i][dp[0].length-1] = dp[i+1][dp[0].length-1];
		for(int i = dp[0].length-2; i > -1 && grid[grid.length-1][i] != '*'; i--)dp[dp.length-1][i] = dp[dp.length-1][i+1];
