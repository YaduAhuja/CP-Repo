...
public class PartitionIt {
    ...
    private static StringBuilder solve(int[] cons) {
        ...
        for (int i = 2; i <= cons[0]; i++) {
            if (isPrime[i] && i * 2 > cons[0]) more.add(i);
            else less.add(i);
        }
        ...
        if (cons[1] >= less.size()) {
            ...
            for (int i = 0; i < more.size() && cons[1] > less.size(); i++) less.add(more.get(i));

            for (int i : less) sb.append(i).append(" ");

        } else if (cons[1] <= more.size()) {
            sb.append("YES\n");

            while (cons[1] < more.size()) more.remove(more.size() - 1);

            for (int i : more) sb.append(i).append(" ");
        } else sb.append("NO");
