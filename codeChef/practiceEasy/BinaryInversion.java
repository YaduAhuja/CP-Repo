...
public class BinaryInversion {
    public static void main(String args[]) throws Exception {
        ...
        while (t-- > 0) {
            ...
            for (int i = 0; i < arr.length; i++) arr[i] = new Pair(br.readLine().trim());
    ...
    private static long getMinInversions(Pair[] arr) {
        for (Pair p : arr) p.oneCount = getOneCount(p.s);
        ...
        for (int i = arr.length - 1; i > -1; i--) {
            ...
            for (int j = temp.s.length() - 1; j > -1; j--)
                if (temp.s.charAt(j) == '0') zeroCount++;
                else ret += zeroCount;
        }
    ...
    private static int getOneCount(String s) {
        int ret = 0;
        for (char c : s.toCharArray()) if (c == '1') ret++;
