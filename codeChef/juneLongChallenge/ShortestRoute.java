...
public class ShortestRoute {
		...
		for (int i = 0; i < t; i++) {
			...
			for (int j = 0; j < n; j++) {
				...
				if (x == 0)
					continue;
				if (x == 1)
					rightMovingTrains.add(j + 1);
				else
					leftMovingTrains.add(j + 1);
			}

			st = new StringTokenizer(br.readLine().trim(), " ");
			for (int j = 0; j < queries.length; j++)
				queries[j] = Integer.parseInt(st.nextToken());
	...
	private static StringBuilder serveQueries(int queries[], ArrayList<Integer> leftMovingTrains, ArrayList<Integer> rightMovingTrains) {
		...
		for(int i = 0; i < queries.length; i++){
			...
			int minTimeLeft = queries[i] > nearestLeftTrain ? Integer.MAX_VALUE : nearestLeftTrain - queries[i];
			int minTimeRight = queries[i] < nearestRightTrain ? Integer.MAX_VALUE : queries[i] - nearestRightTrain;
		
			// System.out.println(minTimeLeft +" "+ minTimeRight);
			
			if(minTimeLeft == Integer.MAX_VALUE && minTimeRight == Integer.MAX_VALUE) sb.append("-1 ");
