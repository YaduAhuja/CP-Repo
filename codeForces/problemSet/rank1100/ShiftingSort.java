...
public class ShiftingSort {
    ...
    private static String getSteps(int[] arr) {
        ...
        for (String s : operations) sb.append(s).append("\n");
        return sb.toString();
    }

    private static int findIndex(int[] arr, int element, int left) {
        for (int i = left; i < arr.length; i++) if (arr[i] == element) return i;
    ...
    private static void shift(int[] arr, int l, int d) {
        ...
        for (int i = l; i < arr.length; i++) {
            if (i - d < l) temp[temp.length + i - l - d] = arr[i];
            else temp[i - l - d] = arr[i];
        }
