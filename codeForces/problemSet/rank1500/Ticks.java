...
public class Ticks {
    ...
    public static void main(String args[]) throws Exception {
        ...
        while (t-- > 0) {
            ...
            for (int i = 0; i < matrix.length; i++) matrix[i] = br.readLine().trim().toCharArray();
    ...
    private static String isPossible(char[][] matrix, int k) {
        ...
        for (int i = matrix.length - 1; i >= 0; i--) {
            for (int j = 0; j < matrix[i].length; j++) {
                ...
                if (size >= k) color(i, j, size);
            }
        }

        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[0].length; j++) {
                if (matrix[i][j] == '*' && !visited[i][j]) return "NO";
            }
        }
        return "YES";
    }
    ...
    private static int getTickSize(char matrix[][], int i, int j) {
        ...
        while (getValue(matrix, i - ind, j + ind) == '*'
                && getValue(matrix, i - ind, j - ind) == '*') {
            size++;
            ind++;
        }
        return size;
