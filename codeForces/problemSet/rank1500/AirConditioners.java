...
public class AirConditioners {
    ...
    private static String getLandTemps(int N, int airConditioners[], int temps[]) {
        ...
        for (int i = 1; i < left.length; i++) left[i] = Math.min(left[i - 1] + 1, left[i]);

        for (int i = N - 2; i >= 0; i--) right[i] = Math.min(right[i + 1] + 1, right[i]);

        for (int i = 0; i < ret.length; i++) ret[i] = Math.min(left[i], right[i]);
